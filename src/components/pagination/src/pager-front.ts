import {computed, watchEffect, watch, reactive, Ref} from "vue";
...
export const pagerFront = (
    ...
    if($$BePaginProps.isFront) {
        ...
        const pageFrontData = computed(():Array<number> =>{
            ...
            for (let i:number = 1; i <= maxPageNum.value; i++) {
                let slice:Array<number> = $$BePaginProps.pageData.slice((i - 1) * pageSize, i * pageSize);
            ...
            // 初始时，返回切片数据
            if(currentPage === 1) { inst.emit("updatePage", {data: sliceList.get(1)})}
        ...
        const prePageFront = ():void => {
            let currentPage:number = Number($$BePaginProps.currentPage) > maxPageNum.value ? maxPageNum.value : Number($$BePaginProps.currentPage);
