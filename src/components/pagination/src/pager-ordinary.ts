...
/*export default {
  computed: {
    ...
    pagers() {
      ...
      const pageCount = Number($$BePagination.pageCount);
      this.maxPageNum = Math.ceil(pageCount / Number($$BePagination.pageSize))
      const currentPage = Number($$BePagination.currentPage) > this.maxPageNum ? this.maxPageNum : Number($$BePagination.currentPage);
      // 当小于用户定义的显示页数 大于 用户总页数和显示数计算出来的总页数时，使用计算的总页数
      pagerCount = (Number($$BePagination.pageSize) / pageCount) <= pagerCount ? (Number($$BePagination.pageSize)/ pageCount) : pagerCount;
