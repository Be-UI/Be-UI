...
*/
<template>
    <be-popover :trigger="focusTrigger ? 'click':'none'"
                placement="bottom"
                ref="beInputPopover"
...
export default defineComponent({
    ...
    setup(props, ctx) {
        ...
        /**************************************** 输入建议下拉框方法 *******************************************/
            // 触发方式
        const focusTrigger = ref<boolean>(props.focusTrigger)
        // 输入建议下拉样式
         ...
         * 获取输入建议
         */
        const getSuggestions = (cb: Function = () => {
        }): void => {
            nextTick(() => {
                // 沒有缓存就請求數據
                if (props.fetchSuggestions) {
        ...
        const modelValComp = computed(()=>{return props.modelValue})
