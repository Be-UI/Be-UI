...
export default defineComponent({
    ...
    setup(){
      ...
      const internalInstance = getCurrentInstance() as IBreadcrumbInst
        let slots:Array<IBreadcrumbItemVnode> = internalInstance.slots.default ? internalInstance.slots.default() :[]
        // 给插槽最后一个虚拟dom打上标记，在点击跳转时能够识别阻止
        if(slots.length > 0 && (slots[slots.length - 1].type as IBreadcrumb).name === 'BeBreadcrumbItem'){
