...
export default defineComponent({
  ...
  setup(props, ctx) {
    ...
    // 处理获得常规分页页码数据
    const pagers = computed((): Array<number> => {
      return pagersList($$BePaginProps, maxPageNum, showPrevMore, showNextMore)
    })
    // 处理获得动态分页页码数据
    const pagersDynamic = computed((): Array<number> => {
    ...
    const sliceList = ref(pagerFrontParam ? pagerFrontParam.sliceList : null)
    const frontList = ref(pagerFrontParam ? pagerFrontParam.frontList : null)
    const nextPageFront = ref(pagerFrontParam ? pagerFrontParam.nextPageFront : null)
    const prePageFront = ref(pagerFrontParam ? pagerFrontParam.prePageFront : null)
      ...
      if (!jump) {
        ...
        if (
          target.className
            ...
            && (target.className.includes('pageLast')
              || target.className.includes('pageFirst'))
        ) {
          const childNodes = target.childNodes[0] as HTMLElement
          if (childNodes.className.includes('quickprev')) {
            newPage = currentPage - pagerCountOffset
            // 小于第一页 修正
            newPage = newPage < 1 ? 1 : newPage
          } else if (childNodes.className.includes('quicknext')) {
            newPage = currentPage + pagerCountOffset
            // 大于第末页 修正
        ...
        if (
          target.className
            ...
            && classNameSVG.className.baseVal.toString().includes('be-icon')
        ) {
          if (
            target.parentElement
              && target.parentElement.className.includes('quickprev')
          ) {
            newPage = currentPage - pagerCountOffset
            // 小于第一页 修正
            newPage = newPage < 1 ? 1 : newPage
            ...
            target.parentElement
              && target.parentElement.className.includes('quicknext')
          ) {
            newPage = currentPage + pagerCountOffset
            // 大于第末页 修正
            newPage = newPage > maxPageNum.value ? maxPageNum.value : newPage
    ...
    /** ************************************ 上下缩略翻页样式控制 ******************************/
    const quicknextIconClass = ref<string>('more')
    const quickprevIconClass = ref<string>('more')
    const hoverNextIconClass = ref<string>('#303133')
    const hoverprevIconClass = ref<string>('#303133')
    watchEffect(() => {
