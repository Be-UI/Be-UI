...
import type { IContextMenu, IPosition, IPositionStr, MyWindow, refNode } from './be-contextmenu-type'
...
export default defineComponent({
  ...
  setup(props, ctx) {
    ...
    /** ********************************** 菜單位置計算方法 *****************************/
    // 位置设置
    const style = reactive<IPosition>({ top: 0, left: 0 })
    /**
       * 处理右键点击方法
    ...
    function handleReferenceContextmenu(evt: Event) {
      ...
      show()
      nextTick(() => {
        const contextmenuPosition: IPositionStr = {
          top: eventY,
          left: eventX,
        }
        // 设置自动布局显示则计算布局位置 并更新
