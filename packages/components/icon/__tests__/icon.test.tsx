...
describe('test-be-icon-props', () => {
  test('width-height-number', () => {
    ...
    const iconElm: HTMLElement | null = wrapper.element.querySelector('.be-icon--container')
    console.log(iconElm)
    expect(iconElm?.getAttribute('width')).toBe(`99`)
    expect(iconElm?.getAttribute('height')).toBe(`99`)
  })
  ...
  test('color', () => {
    ...
    const iconUseTagElm: Element | undefined =
      wrapper.element.querySelector('.be-icon--container')?.children[0]
...
import { BeIconComponents } from '../index'

const panda: object = BeIconComponents('panda', pandaIcon)
