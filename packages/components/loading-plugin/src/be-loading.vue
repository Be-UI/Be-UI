  ...
  setup(props) {
    ...
    const setText = () => {
      if (props.text) {
        // 根据插槽元素数据，计算文字位置
        nextTick(() => {
          const loaderElem: HTMLElement | null = document.querySelector('.be-load')
          if (!loaderElem)
            return
          const loaderElemHeight = Number(window.getComputedStyle(loaderElem).height.split('px')[0])
    ...
    const getAbsolutePosition = (slotElem: HTMLElement): IPosStyle | null => {
      ...
      const r: number = document.body.offsetWidth - w - l
      const b: number = document.body.offsetHeight - h - t
