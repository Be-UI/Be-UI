...
export default defineComponent({
  ...
  setup(props, ctx) {
    ...
    const handleInput = (): void => {
      const suggestionList = props.suggestionList.length > 0 ? props.suggestionList : listDataCache
      // 點擊觸發時
    ...
    // 輸入建議數據 数组要用ref
    const selectList = ref<Array<any>>(props.suggestionList)
    /**
       * 计算输入建议下拉框位置
      ...
      nextTick(() => {
        selectList.value = value
          ? ordData.filter((val: any) => {
            return (
              val[props.labelValue].toString().toLowerCase().includes(value.toLowerCase())
            )
          })
          : ordData
      })
